...
class TestKnotConstruction:
    ...
    def test_knot_ordering(self):
        ...
        # Should have boundary extensions (not repeated boundary knots)
        # First knot should be less than domain start
        assert knots[0] < 0
        # Last knot should be greater than domain end
        assert knots[-1] > 1
        # Domain boundaries should be in the knot vector
        assert 0 in knots
        assert 1 in knots
