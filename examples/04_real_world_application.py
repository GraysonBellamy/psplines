...
def forecast_with_uncertainty(spline, t_obs, n_forecast=30, method="linear_trend"):
    ...
    # t_extended = np.concatenate([t_obs, t_forecast])  # Unused but kept for reference
    ...
    if method == "linear_trend":
        ...
        trend_slope = (
            spline.fitted_values[-1] - spline.fitted_values[-last_n]
        ) / last_n

        # Extrapolate
        forecast_values = spline.fitted_values[-1] + trend_slope * np.arange(
            1, n_forecast + 1
        )
...
def main():
    ...
    plt.plot(
        dates, y, "o-", alpha=0.5, markersize=2, color="gray", label="Observed data"
    )
    plt.plot(dates, trend_fitted, "r-", linewidth=3, label="Long-term trend (λ=1000)")
    plt.plot(
        dates,
        spline_medium.fitted_values,
        "b-",
        linewidth=2,
    ...
    if n_anomalies > 0:
        ...
        plt.scatter(
            ...
            marker="x",
            linewidth=3,
            label=f"Detected anomalies ({n_anomalies})",
        )
    ...
    plt.axhline(
        y=threshold, color="red", linestyle="--", alpha=0.7, label=f"±{2.5}σ threshold"
    )
    ...
    plt.plot(
        dates[recent_idx],
        spline_medium.fitted_values[recent_idx],
        "b-",
        linewidth=2,
        label="Fitted values",
    )
    ...
    # Plot forecast
    forecast_dates = [
        dates[-1] + timedelta(days=i) for i in range(1, len(forecast_values) + 1)
    ]
    ...
    plt.axvline(
        x=dates[-1], color="black", linestyle=":", alpha=0.7, label="Forecast start"
    )
    ...
    plt.plot(
        recent_dates, recent_y, "o", alpha=0.4, markersize=3, color="gray", label="Data"
    )
    ...
    if n_anomalies > 0:
        ...
        print(
            f"     - Dates: {', '.join(anomaly_dates_str[:5])}{'...' if n_anomalies > 5 else ''}"
        )

    print("\n   Forecast Summary:")
    print(f"     - Forecast horizon: {len(forecast_values)} days")
    print(
        f"     - Final forecast: {forecast_values[-1]:.2f} ± {1.96 * forecast_se[-1]:.2f}"
    )
    print(
        f"     - Trend direction: {'Increasing' if forecast_values[-1] > forecast_values[0] else 'Decreasing'}"
    )
