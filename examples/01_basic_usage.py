...
def main():
    ...
    axes[0, 0].plot(
        x_pred,
        y_true[::2] if len(y_true) == len(x_pred) else np.sin(x_pred),
        "g-",
        label="True function",
        linewidth=1,
    )
    ...
    axes[1, 1].plot(
        x_pred, -np.sin(x_pred), "g--", alpha=0.7, label="True 2nd derivative"
    )
